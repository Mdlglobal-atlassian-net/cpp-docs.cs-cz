...
Funkce **freopen_s** zavře soubor, který je aktuálně přidružený ke *streamu* , a znovu přiřadí *datový proud* k souboru určenému *cestou*. **_wfreopen_s** je verze **_freopen_s**s velkým znakem; argumenty *cesty* a *režimu* , které se mají **_wfreopen_s** , jsou řetězce s velkým znakem. **_wfreopen_s** a **_freopen_s** se chovají identicky jinak.
