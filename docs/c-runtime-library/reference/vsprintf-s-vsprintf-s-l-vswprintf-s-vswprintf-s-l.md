...
Zapíše formátovaný výstup pomocí ukazatele na seznam argumentů. Toto jsou verze [vsprintf –, _vsprintf_l –, vswprintf –, _vswprintf_l –, \__vswprintf_l –](../../c-runtime-library/reference/vsprintf-vsprintf-l-vswprintf-vswprintf-l-vswprintf-l.md) vylepšení zabezpečení, jak je popsáno v [funkce zabezpečení v CRT](../../c-runtime-library/security-features-in-the-crt.md).  
...
## <a name="return-value"></a>Návratová hodnota  
 `vsprintf_s` a `vswprintf_s` vrátí počet znaků zapsána, pokud dojde k chybě výstup není včetně ukončující znak hodnoty null nebo záporná hodnota. Pokud `buffer` nebo `format` je ukazatel s hodnotou null, pokud je počet nula nebo pokud řetězec formátu obsahuje neplatné znaky formátování, je obslužná rutina neplatný parametr vyvolána, jak je popsáno v [ověření parametru](../../c-runtime-library/parameter-validation.md). Pokud je povoleno provádění pokračovat, funkce vrátí hodnotu -1 a nastavte `errno` k `EINVAL`.  
...
## <a name="remarks"></a>Poznámky  
 ...
 `vswprintf_s` vyhovuje standardu ISO C pro `vswprintf`, což vyžaduje, aby druhý parametr `count`, typu `size_t`.  
