...
Funkce **_fsopen** otevře soubor určený parametrem *filename* jako datový proud a připraví soubor pro následné sdílené čtení nebo zápis, jak jsou definovány argumenty Mode a *Shflag* . **_wfsopen** je verze **_fsopen**s velkým znakem; argumenty *filename* a *Mode* pro **_wfsopen** jsou řetězce s velkým znakem. **_wfsopen** a **_fsopen** se chovají identicky jinak.
