...
## <a name="return-value"></a>Návratová hodnota  
 Pokud úspěšné, každá z těchto funkcí vrátí znak argument `c`. Pokud `c` nelze posunuto nebo žádné znak byl načten, vstupního datového proudu je zrušeno a `_ungetc_nolock` vrátí `EOF`; `_ungetwc_nolock` vrátí `WEOF`. Pokud `stream` je `NULL`, `EOF` nebo `WEOF` je vrácen a `errno` je nastaven na `EINVAL`.  
