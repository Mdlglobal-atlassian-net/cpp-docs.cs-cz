...
## <a name="remarks"></a>Poznámky  
 ...
 Další informace o konkrétní parametry, které chcete použít a hodnot vrácených vnitřní tyto funkce na od společnosti AMD, najdete v dokumentaci pro `cpuid` instrukce v [programátory architektura AMD64 ruční svazku 3: Pro obecné účely a pokyny systému](http://go.microsoft.com/fwlink/p/?LinkId=510023) a v [revize příručky](http://go.microsoft.com/fwlink/p/?LinkId=510023) rodin specifické procesory. Dokumentace AMD používá podmínky "funkce číslo" a "podfunkce je číslo" pro `function_id` a `subfunction_id` parametry předané EAX a ECX.  
  
 Když `function_id` argument je 0, `cpuInfo[0]` vrátí nejvyšší dostupné rozšířených `function_id` podporovaných procesorem. Výrobce procesoru je zakódován do `cpuInfo[1]`, `cpuInfo[2]`a cpuInfo [3].  
  
 Podpora pro konkrétní instrukce nastavit rozšíření a funkce procesoru je zakódován do `cpuInfo` vráceny výsledky pro vyšší hodnoty function_id. Další informace najdete v tématu příručky uvedený výše a následující příklad kódu.  
