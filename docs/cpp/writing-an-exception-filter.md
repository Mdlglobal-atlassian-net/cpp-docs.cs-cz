...
Všimněte si použití [GetExceptionCode](/windows/win32/Debug/getexceptioncode) k určení výjimky. Tuto funkci je nutné volat uvnitř samotného filtru. `Eval_Exception`nelze volat `GetExceptionCode`, ale musí mít předaný kód výjimky.
...
Podmíněný operátor je užitečnější v situacích, kdy je vhodné, aby se filtr vyhodnotil jako-1, EXCEPTION_CONTINUE_EXECUTION.
