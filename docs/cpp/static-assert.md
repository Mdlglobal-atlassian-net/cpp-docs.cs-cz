...
|Parametr|Popis|
|---------------|-----------------|
|*constant-expression*|Celočíselný konstantní výraz, který lze převést na logickou hodnotu.<br /><br /> Pokud je vyhodnocený výraz nula (false), je zobrazen parametr řetězcového literálu a kompilace se nezdařila s chybou. Pokud je výraz nenulový (true), deklarace **static_assert** nemá žádný vliv.|
...
V následujícím příkladu má deklarace **static_assert** obor názvů. Vzhledem k tomu, že kompilátor zná velikost `void *`typu, je výraz vyhodnocen okamžitě.
...
V následujícím příkladu má deklarace **static_assert** obor třídy. **Static_assert** ověří, zda je parametr šablony prostým typem *starého data* (pod). Kompilátor při deklaraci ověří deklaraci **static_assert** , ale nevyhodnotí `basic_string` parametr *konstantního výrazu* , dokud není vytvořena instance šablony `main()`třídy.
...
V následujícím příkladu má deklarace **static_assert** rozsah bloku. **Static_assert** ověří, zda je velikost struktury VMPage shodná s velikostí paměti virtuálního počítače.
