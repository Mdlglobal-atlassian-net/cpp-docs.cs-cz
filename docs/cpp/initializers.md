---
title: Inicializátory | Dokumentace Microsoftu
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-language
...
-   Deklarace proměnné explicitně zadané jako **extern**. Příklad:  
 ...
 ![Graf rozhodnutí inicializace typů ref](../cpp/media/vc38s71.gif "vc38S71")  
Graf rozhodnutí inicializace typů odkazů  
  
 Odkazy na **volatile** typy (deklarované jako **volatile** *typename *** &** *identifikátor*) mohou být inicializovány s **volatile** objekty stejného typu nebo objekty, které nejsou deklarovány jako **volatile**. Nelze je však, inicializovat pomocí **const** objekty daného typu. Podobně, odkazy na **const** typy (deklarované jako **const** *typename *** &** *identifikátor*) může být inicializovat pomocí **const** objekty stejného typu (nebo čímkoli, kde existuje převod na daný typ nebo pomocí objektů, které nejsou deklarovány jako **const**). Nelze je však, inicializovat pomocí **volatile** objekty daného typu.  
