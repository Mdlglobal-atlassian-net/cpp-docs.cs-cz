---
title: Vlastnosti projektu Clang (Android C++)
ms.date: 10/23/2017
...
| Úroveň upozornění | Vyberte, jak striktní má kompilátor obsahovat chyby kódu.  Další příznaky by se měly přidat přímo k dalším možnostem. (/w,/Weverything). | Vypnout **všechna upozornění** – zakáže všechna upozornění kompilátoru.<br>**Povolit všechna upozornění** – povolí všechna upozornění, včetně všech zakázaných ve výchozím nastavení.<br> |
...
| Striktní aliasy | Předpokládají nejpřísnější pravidla aliasování.  U objektu jednoho typu se nikdy nepředpokládá, že by měl být na stejné adrese jako jiný objekt typu. |
...
| Povolit Advanced SIMD (Neon) | Povoluje generování kódu pro hardware s plovoucí desetinnou čárkou NEON. Platí jenom pro architekturu ARM. |
...
| Zrušit Definice preprocesoru | Určuje jeden nebo více undefined pro preprocesor.  ( *Makro*-U) |
...
| Adresář výstupního souboru předkompilované hlavičky | Určuje adresář pro generovanou předkompilovanou hlavičku. Tento adresář je také přidán do ' další adresáře zahrnutí ' během sestavení |
...
| Kompilovat jako | Umožňuje vybrat možnost jazyka kompilace pro soubory *`.c`* a *`.cpp`* .  Klíčové slovo Default se detekuje na základě rozšíření *`.c`* nebo *`.cpp`* . (-x c, -x c++) | **Výchozí** – výchozí.<br>**Kompilovat jako kód jazyka c** – kompilovat jako kód jazyka c.<br>**Kompilovat jako C++**  kód – kompilovat jako C++ kód.<br> |
| Vynucené vložené soubory | jeden nebo více souborů s vynuceným zahrnutím.     (-include *Name*) |
