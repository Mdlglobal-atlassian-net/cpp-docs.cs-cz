...
|[REFLECTED_COMMAND_CODE_HANDLER](#reflected_command_code_handler)|Namapuje zrcadlenou zprávu WM_COMMAND na funkci obslužné rutiny na základě kódu oznámení.|
...
|[REFLECTED_NOTIFY_CODE_HANDLER](#reflected_notify_code_handler)|Namapuje zrcadlenou zprávu WM_NOTIFY na funkci obslužné rutiny na základě kódu oznámení.|
...
Makro [END_MSG_MAP](#end_msg_map) označuje konec mapy zpráv. Všimněte si, že je vždy pouze jedna instance BEGIN_MSG_MAP a END_MSG_MAP.
...
|[COMMAND_RANGE_HANDLER](#command_range_handler)|Mapuje souvislý rozsah zpráv WM_COMMAND na funkci obslužné rutiny na základě identifikátoru položky nabídky, ovládacího prvku nebo akcelerátoru.|
|[NOTIFY_HANDLER](#notify_handler)|Mapuje zprávu WM_NOTIFY na funkci obslužné rutiny na základě kódu oznámení a identifikátoru ovládacího prvku.|
|[NOTIFY_ID_HANDLER](#notify_id_handler)|Mapuje zprávu WM_NOTIFY na funkci obslužné rutiny na základě identifikátoru ovládacího prvku.|
|[NOTIFY_CODE_HANDLER](#notify_code_handler)|Mapuje zprávu WM_NOTIFY na funkci obslužné rutiny na základě kódu oznámení.|
|[NOTIFY_RANGE_HANDLER](#notify_range_handler)|Mapuje souvislý rozsah zpráv WM_NOTIFY na funkci obslužné rutiny na základě identifikátoru ovládacího prvku.|
...
Když objekt obdrží zprávu WM_PAINT, zpráva se přesměruje na `CMyExtWindow::OnPaint` konkrétní zpracování. `CMyExtWindow` Pokud `OnPaint` indikuje, že zpráva vyžaduje další zpracování, zpráva se pak přesměruje na výchozí mapu zpráv v `CMyBaseWindow`.
...
CHAIN_MSG_MAP_ALT směruje zprávy na alternativní mapu zpráv v základní třídě. Tuto alternativní mapu zpráv musíte deklarovat pomocí [ALT_MSG_MAP (msgMapID)](#alt_msg_map). Chcete-li směrovat zprávy do výchozího mapování zpráv základní třídy (deklarované pomocí [BEGIN_MSG_MAP](#begin_msg_map)), použijte CHAIN_MSG_MAP. Příklad naleznete v tématu [CHAIN_MSG_MAP](#chain_msg_map).

> [!NOTE]
>  Vždy zahajte mapu zpráv pomocí BEGIN_MSG_MAP. Pak můžete deklarovat následné alternativní mapy zpráv pomocí ALT_MSG_MAP. Makro [END_MSG_MAP](#end_msg_map) označuje konec mapy zpráv. Každá mapa zpráv musí mít přesně jednu instanci třídy BEGIN_MSG_MAP a END_MSG_MAP.
...
CHAIN_MSG_MAP směruje zprávy na výchozí mapu zprávy základní třídy (deklarovaný s [BEGIN_MSG_MAP](#begin_msg_map)). Chcete-li směrovat zprávy do alternativní mapy zpráv základní třídy (deklarované pomocí [ALT_MSG_MAP](#alt_msg_map)), použijte [CHAIN_MSG_MAP_ALT](#chain_msg_map_alt).
...
- Pokud procedura okna používá `CMyClass`výchozí mapu zpráv a `OnPaint` nezpracovává zprávu, je zpráva směrována na `CMyBaseClass`výchozí mapu zpráv pro zpracování.

- Pokud se v rámci okna používá první alternativní mapování zpráv v `CMyClass`nástroji, jsou všechny zprávy směrovány na `CMyBaseClass`výchozí mapu zpráv.

- Pokud procedura okna používá `CMyClass`druhé alternativní mapování zpráv a `OnChar` nezpracovává zprávu, zpráva se přesměruje na zadané alternativní mapování zpráv v `CMyBaseClass`. `CMyBaseClass`musí mít deklaraci tohoto mapování zpráv s ALT_MSG_MAP (1).
...
> [!NOTE]
>  Vždy zahajte mapu zpráv pomocí [BEGIN_MSG_MAP](#begin_msg_map). Pak můžete deklarovat následné alternativní mapy zpráv pomocí ALT_MSG_MAP. Makro [END_MSG_MAP](#end_msg_map) označuje konec mapy zpráv. Každá mapa zpráv musí mít přesně jednu instanci třídy BEGIN_MSG_MAP a END_MSG_MAP.
...
CHAIN_MSG_MAP_MEMBER směruje zprávy na výchozí mapu zpráv datového členu (deklarovaný s [BEGIN_MSG_MAP](#begin_msg_map)). Chcete-li směrovat zprávy na alternativní mapu zpráv datového člena (deklarovaný s [ALT_MSG_MAP](#alt_msg_map)), použijte [CHAIN_MSG_MAP_ALT_MEMBER](#chain_msg_map_alt_member).
...
- Pokud procedura okna používá `CMyClass`výchozí mapu zpráv a `OnPaint` nezpracovává zprávu, je zpráva směrována na `m_obj`výchozí mapu zpráv pro zpracování.

- Pokud se v rámci okna používá první alternativní mapování zpráv v `CMyClass`nástroji, jsou všechny zprávy směrovány na `m_obj`výchozí mapu zpráv.

- Pokud procedura okna používá `CMyClass`druhé alternativní mapování zpráv a `OnChar` nezpracovává zprávu, zpráva se přesměruje na `m_obj`určenou alternativní mapu zpráv. Třída `CMyContainedClass` musí deklarovat tuto mapu zpráv pomocí ALT_MSG_MAP (1).
...
Mapa `bHandled` zpráv je nastavena na hodnotu true `CommandHandler` před voláním metody. Pokud `CommandHandler` Tato zpráva nebude plně zpracována, měla by být `bHandled` nastavena na hodnotu false, aby označovala, že zpráva bude potřebovat další zpracování.

> [!NOTE]
>  Vždy zahajte mapu zpráv pomocí [BEGIN_MSG_MAP](#begin_msg_map). Pak můžete deklarovat následné alternativní mapy zpráv pomocí [ALT_MSG_MAP](#alt_msg_map). Makro [END_MSG_MAP](#end_msg_map) označuje konec mapy zpráv. Každá mapa zpráv musí mít přesně jednu instanci třídy BEGIN_MSG_MAP a END_MSG_MAP.

Kromě COMMAND_HANDLER můžete použít [MESSAGE_HANDLER](#message_handler) k mapování zprávy WM_COMMAND bez ohledu na identifikátor nebo kód. V takovém případě `MESSAGE_HANDLER(WM_COMMAND, OnHandlerFunction)` budou všechny zprávy WM_COMMAND směrovat na `OnHandlerFunction`.
...
Podobně jako [COMMAND_RANGE_HANDLER](#command_range_handler), ale mapuje [WM_COMMAND](/windows/win32/menurc/wm-command) zprávy pomocí konkrétního kódu oznámení z rozsahu ovládacích prvků na jedinou funkci obslužné rutiny.
...
Podobně jako [COMMAND_HANDLER](#command_handler), ale mapují zprávy [WM_COMMAND](/windows/win32/menurc/wm-command) z rozsahu ovládacích prvků na jedinou funkci obslužné rutiny.
...
DECLARE_EMPTY_MSG_MAP je praktické makro, které volá makra [BEGIN_MSG_MAP](#begin_msg_map) a [END_MSG_MAP](#end_msg_map) , aby se vytvořila prázdná mapa zpráv:

[!code-cpp[NVC_ATL_Windowing#122](../../atl/codesnippet/cpp/message-map-macros-atl_7.h)]

##  <a name="default_reflection_handler"></a>DEFAULT_REFLECTION_HANDLER

Poskytuje výchozí obslužnou rutinu pro podřízené okno (ovládací prvek), které obdrží odrážetelné zprávy. Obslužná rutina bude správně předávat nezpracované zprávy `DefWindowProc`do.
...
Vždy použijte makro [BEGIN_MSG_MAP](#begin_msg_map) k označení začátku mapy zprávy. Použijte [ALT_MSG_MAP](#alt_msg_map) k deklaraci dalších alternativních map zpráv.

Všimněte si, že je vždy pouze jedna instance BEGIN_MSG_MAP a END_MSG_MAP.
...
Mapa `bHandled` zpráv je nastavena na hodnotu true `MessageHandler` před voláním metody. Pokud `MessageHandler` Tato zpráva nebude plně zpracována, měla by být `bHandled` nastavena na hodnotu false, aby označovala, že zpráva bude potřebovat další zpracování.

> [!NOTE]
>  Vždy zahajte mapu zpráv pomocí [BEGIN_MSG_MAP](#begin_msg_map). Pak můžete deklarovat následné alternativní mapy zpráv pomocí [ALT_MSG_MAP](#alt_msg_map). Makro [END_MSG_MAP](#end_msg_map) označuje konec mapy zpráv. Každá mapa zpráv musí mít přesně jednu instanci třídy BEGIN_MSG_MAP a END_MSG_MAP.

Kromě MESSAGE_HANDLER můžete použít [COMMAND_HANDLER](#command_handler) a [NOTIFY_HANDLER](#notify_handler) k mapování zpráv [WM_COMMAND](/windows/win32/menurc/wm-command) a [WM_NOTIFY](/windows/win32/controls/wm-notify) v uvedeném pořadí.
...
Podobně jako [MESSAGE_HANDLER](#message_handler), ale mapuje řadu zpráv systému Windows k jedné funkci obslužné rutiny.
...
Mapa `bHandled` zpráv je nastavena na hodnotu true `NotifyHandler` před voláním metody. Pokud `NotifyHandler` Tato zpráva nebude plně zpracována, měla by být `bHandled` nastavena na hodnotu false, aby označovala, že zpráva bude potřebovat další zpracování.

> [!NOTE]
>  Vždy zahajte mapu zpráv pomocí [BEGIN_MSG_MAP](#begin_msg_map). Pak můžete deklarovat následné alternativní mapy zpráv pomocí [ALT_MSG_MAP](#alt_msg_map). Makro [END_MSG_MAP](#end_msg_map) označuje konec mapy zpráv. Každá mapa zpráv musí mít přesně jednu instanci třídy BEGIN_MSG_MAP a END_MSG_MAP.

Kromě NOTIFY_HANDLER můžete použít [MESSAGE_HANDLER](#message_handler) k mapování zprávy WM_NOTIFY bez ohledu na identifikátor nebo kód. V takovém případě `MESSAGE_HANDLER(WM_NOTIFY, OnHandlerFunction)` budou všechny zprávy WM_NOTIFY směrovat na `OnHandlerFunction`.
...
Podobně jako [NOTIFY_RANGE_HANDLER](#notify_range_handler), ale mapuje [WM_NOTIFY](/windows/win32/controls/wm-notify) zprávy pomocí konkrétního kódu oznámení z rozsahu ovládacích prvků na jedinou funkci obslužné rutiny.
...
Podobně jako [NOTIFY_HANDLER](#notify_handler), ale mapují zprávy [WM_NOTIFY](/windows/win32/controls/wm-notify) z rozsahu ovládacích prvků na jedinou funkci obslužné rutiny.
...
Podobně jako [NOTIFY_CODE_HANDLER](#notify_code_handler), ale oznámení o mapách se projeví z nadřazeného okna.
...
Podobně jako [NOTIFY_HANDLER](#notify_handler), ale oznámení o mapách se projeví z nadřazeného okna.
...
Podobně jako [NOTIFY_ID_HANDLER](#notify_id_handler), ale oznámení o mapách se projeví z nadřazeného okna.
...
Podobně jako [NOTIFY_RANGE_CODE_HANDLER](#notify_range_code_handler), ale oznámení o mapách se projeví z nadřazeného okna.
...
Podobně jako [NOTIFY_RANGE_HANDLER](#notify_range_handler), ale oznámení o mapách se projeví z nadřazeného okna.
