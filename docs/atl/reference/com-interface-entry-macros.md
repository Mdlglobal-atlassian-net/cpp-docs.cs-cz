...
ms.locfileid: "78855663"
---
# <a name="com_interface_entry-macros"></a>COM_INTERFACE_ENTRY makra

Tato makra zadávají rozhraní objektu do své mapy modelu COM, aby k nim bylo možné přihlédnout pomocí `QueryInterface`. Pořadí záznamů v mapě modelu COM je pořadí, ve kterém bude při `QueryInterface`zkontrolováno porovnání identifikátoru IID.
...
|[COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate)|Pokud je rozhraní identifikované *identifikátorem IID* dotazováno na, `COM_INTERFACE_ENTRY_AGGREGATE` předána `punk`.|
|[COM_INTERFACE_ENTRY_AGGREGATE_BLIND](#com_interface_entry_aggregate_blind)|Stejné jako u [COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate), s výjimkou toho, že dotazování pro jakýkoli identifikátor IID má za následek předání dotazu do *punk*.|
|[COM_INTERFACE_ENTRY_AUTOAGGREGATE](#com_interface_entry_autoaggregate)|Stejné jako u [COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate), s výjimkou případů, kdy *punk* má hodnotu null, automaticky vytvoří agregaci popsanou v *CLSID*.|
|[COM_INTERFACE_ENTRY_AUTOAGGREGATE_BLIND](#com_interface_entry_autoaggregate_blind)|Stejné jako u [COM_INTERFACE_ENTRY_AUTOAGGREGATE](#com_interface_entry_autoaggregate), s výjimkou toho, že dotazování pro jakýkoli identifikátor IID má za následek předání dotazu do *punk*a pokud *punk* má hodnotu null, automatické vytvoření agregace popsané *identifikátorem CLSID*.|
...
|[COM_INTERFACE_ENTRY_FUNC](#com_interface_entry_func)|Obecný mechanismus, který se připojí k logice `QueryInterface` ATL.|
...
Například pokud odvozujete objekt třídy ze dvou duálních rozhraní, vystavíte `IDispatch` pomocí COM_INTERFACE_ENTRY2, protože `IDispatch` lze získat z jednoho z rozhraní.
...
Pokud je rozhraní identifikované *identifikátorem IID* dotazováno na, COM_INTERFACE_ENTRY_AGGREGATE předána do *punk*.
...
*punk*<br/>
pro Název ukazatele `IUnknown`.

### <a name="remarks"></a>Poznámky

Předpokládá se, že parametr *punk* odkazuje na vnitřní neznámý objekt agregace nebo na hodnotu null. v takovém případě se položka ignoruje. Obvykle byste `CoCreate` agregaci v `FinalConstruct`.
...
Stejné jako u [COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate), s výjimkou toho, že dotazování pro jakýkoli identifikátor IID má za následek předání dotazu do *punk*.
...
Stejné jako u [COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate), s výjimkou případů, kdy *punk* má hodnotu null, automaticky vytvoří agregaci popsanou v *CLSID*.
...
*punk*<br/>
pro Název ukazatele `IUnknown`. Musí být členem třídy, která obsahuje mapu modelu COM.
...
Stejné jako u [COM_INTERFACE_ENTRY_AUTOAGGREGATE](#com_interface_entry_autoaggregate), s výjimkou toho, že dotazování pro jakýkoli identifikátor IID má za následek předání dotazu do *punk*a pokud *punk* má hodnotu null, automatické vytvoření agregace popsané *identifikátorem CLSID*.
...
Identifikátor IID rozhraní bude vytvořen připojením *x* k `IID_`. Například pokud je *x* `IPersistStorage`, IID bude `IID_IPersistStorage`.
...
*punk*<br/>
pro Název ukazatele `IUnknown`. Musí být členem třídy, která obsahuje mapu modelu COM. By měl být v konstruktoru objektu třídy inicializován na hodnotu NULL.
...
Rozhraní pro odložení je implementováno jako samostatný objekt, jehož instance je vytvořena pokaždé, když je na něj dotazováno rozhraní, které představuje. Obvykle sestavíte rozhraní jako nejenom v případě, že se rozhraní používá zřídka, protože to ukládá ukazatel vtable do každé instance hlavního objektu. Trhlina se odstraní, když se jejich počet odkazů stane nula. Třída implementující trhlinu by měla být odvozena od `CComTearOffObjectBase` a mít vlastní mapu COM.
...
Při volání funkce `pv` odkazuje na objekt třídy. Parametr *riid* odkazuje na rozhraní, na které se dotazuje, `ppv` je ukazatel na umístění, kde by měla funkce Uložit ukazatel na rozhraní a *DW* je parametr, který jste zadali v položce. Funkce by měla nastavit \* `ppv` na hodnotu NULL a vracet E_NOINTERFACE nebo S_FALSE, pokud se rozhodne, že nevrátí rozhraní. Při E_NOINTERFACE ukončí zpracování mapy modelu COM. Při S_FALSE pokračuje zpracování mapy COM i v případě, že nebyl vrácen žádný ukazatel rozhraní. Pokud funkce vrátí ukazatel rozhraní, měla by vracet S_OK.
...
Jakákoli chyba způsobí, že zpracování bude pokračovat na mapě modelu COM. Pokud funkce vrátí ukazatel rozhraní, měla by vracet S_OK.
