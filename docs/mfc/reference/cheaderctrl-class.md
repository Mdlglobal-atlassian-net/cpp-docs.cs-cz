...
|Název|Popis|
|----------|-----------------|
|[CHeaderCtrl:: CHeaderCtrl](#cheaderctrl)|Vytvoří objekt `CHeaderCtrl`.|
...
|[CHeaderCtrl:: Create](#create)|Vytvoří ovládací prvek záhlaví a připojí ho k objektu `CHeaderCtrl`.|
|[CHeaderCtrl:: CreateDragImage](#createdragimage)|Vytvoří transparentní verzi obrázku položky v rámci ovládacího prvku záhlaví.|
|[CHeaderCtrl:: CreateEx](#createex)|Vytvoří ovládací prvek záhlaví se zadanými rozšířenými styly Windows a připojí ho k objektu `CListCtrl`.|
|[CHeaderCtrl::D eleteItem](#deleteitem)|Odstraní položku z ovládacího prvku záhlaví.|
|[CHeaderCtrl::D rawItem](#drawitem)|Nakreslí určenou položku ovládacího prvku záhlaví.|
...
|[CHeaderCtrl:: SetFilterChangeTimeout](#setfilterchangetimeout)|Nastaví interval časového limitu mezi časem změny v atributech filtru a odesláním oznámení `HDN_FILTERCHANGE`.|
...
Tento ovládací prvek (a proto třída `CHeaderCtrl`) je k dispozici pouze pro programy, které běží v systémech Windows 95/98 a Windows NT verze 3,51 a novější.
...
- Položka hlavičky přetažení pro změnu pořadí položek záhlaví. Při vytváření objektu `CHeaderCtrl` použijte styl HDS_DRAGDROP.

- Text záhlaví sloupce se průběžně zobrazit během změny velikosti sloupce. Při vytváření objektu `CHeaderCtrl` použijte styl HDS_FULLDRAG.

- Hlavičku horkého sledování, která zvýrazní položku záhlaví, když na ni ukazatel myši najede. Při vytváření objektu `CHeaderCtrl` použijte styl HDS_HOTTRACK.

- Podpora seznamu obrázků. Položky záhlaví mohou obsahovat obrázky uložené v `CImageList` objekt nebo text.
...
Objekt `CHeaderCtrl` vytvoříte ve dvou krocích. Nejprve volejte konstruktor a potom zavolejte `Create`, čímž se vytvoří ovládací prvek záhlaví a připojí se k objektu `CHeaderCtrl`.
...
- CCS_BOTTOM způsobí, že se ovládací prvek umístí na konec oblasti klienta nadřazeného okna a nastaví šířku tak, aby byla stejná jako šířka nadřazeného okna.

- CCS_NODIVIDER znemožňuje vykreslit v horní části ovládacího prvku dva pixely zvýraznění.

- CCS_NOMOVEY způsobí, že ovládací prvek změní velikost a přesune ho vodorovně, ale ne vertikálně, v reakci na zprávu WM_SIZE. Pokud se používá styl CCS_NORESIZE, tento styl se nepoužije. Ovládací prvky záhlaví mají ve výchozím nastavení tento styl.

- CCS_NOPARENTALIGN znemožňuje automatické přesunutí ovládacího prvku na horní nebo dolní okraj nadřazeného okna. Místo toho ovládací prvek zachovává svou polohu v nadřazeném okně Navzdory změnám velikosti nadřazeného okna. Je-li použit také styl CCS_TOP nebo CCS_BOTTOM, Výška je nastavena na výchozí hodnotu, ale pozice a šířka zůstane beze změny.

- CCS_NORESIZE zabrání ovládacímu prvku v použití výchozí šířky a výšky při nastavení počáteční velikosti nebo nové velikosti. Místo toho používá ovládací prvek šířku a výšku zadanou v žádosti o vytvoření nebo změnu velikosti.
...
Objekt `CImageList`, na který vracené body ukazatele jsou dočasným objektem a který je odstraněn při příštím zpracování v čase nečinnosti.
...
Ve výchozím nastavení tato členská funkce neprovede žádnou akci. Přepište tuto členskou funkci pro implementaci vykreslování pro objekt `CHeaderCtrl` vykreslený vlastníkem.
...
*bDiscardChanges*<br/>
Hodnota, která určuje, jak se mají zpracovat změny úprav uživatele, pokud je uživatel v procesu úprav filtru při odeslání [HDM_EDITFILTER](/windows/win32/Controls/hdm-editfilter) zprávy.
...
Následující příklad kódu definuje proměnnou, `m_headerCtrl`, která se používá pro přístup k aktuálnímu ovládacímu prvku hlavičky. Tato proměnná se používá v následujícím příkladu.

[!code-cpp[NVC_MFC_CHeaderCtrl_s4#6](../../mfc/reference/codesnippet/cpp/cheaderctrl-class_9.h)]

### <a name="example"></a>Příklad

Následující příklad kódu ukazuje metody `SetFocusedItem` a `GetFocusedItem`. V dřívější části kódu jsme vytvořili ovládací prvek záhlaví s pěti sloupci. Oddělovač sloupců však lze přetáhnout, takže sloupec není viditelný. Následující příklad nastaví a pak potvrdí poslední záhlaví sloupce jako položku fokusu.
...
Tato členská funkce implementuje chování zprávy Win32 [HDM_GETIMAGELIST](/windows/win32/Controls/hdm-getimagelist), jak je popsáno v Windows SDK. Objekt `CImageList`, na který vracené body ukazatele jsou dočasným objektem a který je odstraněn při příštím zpracování v čase nečinnosti.
...
*pHeaderItem*<br/>
Ukazatel na strukturu [HDITEM](/windows/win32/api/commctrl/ns-commctrl-hditemw) , která obdrží novou položku. Tato struktura se používá s členskými funkcemi `InsertItem` a `SetItem`. Všechny příznaky nastavené v prvku `mask` zajišťují, aby hodnoty v odpovídajících prvcích byly po návratu správně vyplněny. Pokud je prvek `mask` nastaven na hodnotu nula, hodnoty v ostatních prvcích struktury jsou nevýznamné.
...
|Parametr|Popis|
|---------------|-----------------|
|*iItem*|pro Index položky záhlaví s hodnotou nula, jejíž styl je HDF_SPLITBUTTON. Další informace naleznete v tématu `fmt` člen struktury [HDITEM](/windows/win32/api/commctrl/ns-commctrl-hditemw) .|
...
Následující příklad kódu ukazuje metodu `GetItemDropDownRect`. V dřívější části kódu jsme vytvořili ovládací prvek záhlaví s pěti sloupci. Následující příklad kódu kreslí kolem umístění v prvním sloupci, který je vyhrazen pro tlačítko pro rozevírací seznam záhlaví, 3D obdélník.
...
Následující příklad kódu ukazuje metodu `HitTest`. V dřívější části tohoto příkladu kódu jsme vytvořili ovládací prvek záhlaví s pěti sloupci. Oddělovač sloupců však lze přetáhnout, takže sloupec není viditelný. Tento příklad nahlásí index sloupce, pokud je viditelný, a-1, pokud sloupec není viditelný.
...
*pImageList*<br/>
Ukazatel na objekt `CImageList` obsahující seznam obrázků, který má být přiřazen ovládacímu prvku záhlaví.
...
Tato členská funkce implementuje chování zprávy Win32 [HDM_SETIMAGELIST](/windows/win32/Controls/hdm-setimagelist), jak je popsáno v Windows SDK. Objekt `CImageList`, na který vracené body ukazatele jsou dočasným objektem a který je odstraněn při příštím zpracování v čase nečinnosti.
