...
Třídy klíčového rozhraní odvozené od `CCmdTarget` zahrnují [CView](../../mfc/reference/cview-class.md), [CWinApp](../../mfc/reference/cwinapp-class.md), [objektu CDocument](../../mfc/reference/cdocument-class.md), [CWnd](../../mfc/reference/cwnd-class.md)a [CFrameWnd](../../mfc/reference/cframewnd-class.md). Pokud máte v úmyslu pro zpracování zpráv novou třídou, odvodit třídu z jedné z těchto @no__t tříd odvozených od -0. Třídu můžete přímo odvodit z `CCmdTarget`.

Přehled cílů příkazů a směrování @no__t 0 najdete v tématech [cíle příkazů](../../mfc/command-targets.md), [směrování příkazů](../../mfc/command-routing.md)a [mapování zpráv](../../mfc/mapping-messages.md).
...
Mapy odeslání, podobně jako mapy zpráv, se používají k vystavení funkcí OLE Automation `IDispatch`. Po vystavení tohoto rozhraní můžou další aplikace (například Visual Basic) volat do vaší aplikace.
...
Rozhraní volá tuto funkci, aby zobrazila uživatele, který je zaneprázdněný, například když se objekt `CDocument` načte nebo uloží do souboru.

Akce `BeginWaitCursor` nejsou vždy platné mimo jednu obslužnou rutinu zpráv, protože jiné akce, jako je například zpracování `OnSetCursor`, mohou změnit kurzor.

Voláním `EndWaitCursor` obnovíte předchozí kurzor.
...
Volejte tuto členskou funkci v konstruktoru vašeho objektu odvozeného @no__t -0, pokud poskytuje informace o typu.
...
TRUE, pokud objekt podporuje alespoň jednu akci OLE (v takovém případě \* *ppenumOleVerb* odkazuje na rozhraní enumerátoru `IEnumOLEVERB`), jinak false.
...
Voláním této funkce namapujete ukazatel @no__t 0, přijatý od členských funkcí automatizace třídy, do objektu `CCmdTarget`, který implementuje rozhraní objektu `IDispatch`.
...
*lpDispatch*<br/>
Ukazatel na objekt @no__t 0.

### <a name="return-value"></a>Návratová hodnota

Ukazatel na objekt `CCmdTarget` přidružený k *LPDISPATCH*. Tato funkce vrátí hodnotu NULL, pokud objekt `IDispatch` není rozpoznán jako objekt Microsoft Foundation Class `IDispatch`.

### <a name="remarks"></a>Poznámky

Výsledek této funkce je inverzní hodnota volání členské funkce `GetIDispatch`.
...
Odvozené třídy by měly přepsat tuto členskou funkci (Pokud není přepsána, `GetDispatchIID` vrátí FALSE). Viz [COleControl –](../../mfc/reference/colecontrol-class.md).

##  <a name="getidispatch"></a>CCmdTarget:: GetIDispatch

Zavolejte tuto členskou funkci, aby načetla ukazatel `IDispatch` z metody automatizace, která buď vrací ukazatel `IDispatch`, nebo přebírá ukazatel `IDispatch` odkazem.
...
Odvozené třídy by měly přepsat tuto funkci, aby vracely počet zadaných rozhraní informací o typu (0 nebo 1). Pokud není přepsán, `GetTypeInfoCount` vrátí hodnotu 0. K přepsání použijte makro [IMPLEMENT_OLETYPELIB](../../mfc/reference/type-library-access.md#implement_oletypelib) , které také implementuje `GetTypeLib` a `GetTypeLibCache`.
...
*ppTypeInfo*<br/>
Ukazatel na ukazatel na rozhraní @no__t 0.
...
Tato funkce je volána implementací `IDispatch::Invoke` knihovny MFC k určení, zda lze vyvolat danou metodu automatizace (identifikovanou identifikátorem *DISPID*).
...
Pokud `IsInvokeAllowed` vrátí hodnotu TRUE, `Invoke` pokračuje v volání metody; v opačném případě `Invoke` selže, vrátí se E_UNEXPECTED.
...
Pomocí `IsResultExpected` zjistíte, zda klient očekává vrácenou hodnotu z volání funkce automatizace.
...
`IsResultExpected` vrátí nenulovou hodnotu, pokud je volána, když volání funkce automatizace neprobíhá.
...
*pHandlerInfo*<br/>
Pokud hodnota není NULL, `OnCmdMsg` vyplní *PMF* členů struktury *pHandlerInfo* místo odeslání příkazu. Tento parametr obvykle by měl mít hodnotu NULL.
...
V době běhu `OnCmdMsg` odesílá příkaz do jiných objektů nebo zpracovává příkaz sám, a to voláním kořenové třídy `CCmdTarget::OnCmdMsg`, která provádí vlastní vyhledávání map zpráv. Úplný popis výchozího směrování příkazů najdete v [tématech o zpracování a mapování zpráv](../../mfc/message-handling-and-mapping.md).
...
[Objektu CDocument – třída](../../mfc/reference/cdocument-class.md)<br/>
