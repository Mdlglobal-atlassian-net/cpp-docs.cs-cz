...
## <a name="members"></a>Členové  
 Členské funkce `CUIntArray` jsou podobné funkce člena třídy [CObArray](../../mfc/reference/cobarray-class.md). Z důvodu této podobnosti, můžete použít `CObArray` referenční dokumentace pro konkrétní funkce člen. Po zobrazení `CObject` UINT nahrazení ukazatele jako parametr funkce nebo návratovou hodnotu.  
...
## <a name="remarks"></a>Poznámky  
 Celé číslo bez znaménka nebo UINT, se liší od slova a doublewords v tom, že fyzické velikosti UINT, můžete změnit v závislosti na cíl provozní prostředí. UINT je stejnou velikost jako doubleword.  
