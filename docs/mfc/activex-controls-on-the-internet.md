---
title: Ovládací prvky Activexna Internetu | Dokumentace Microsoftu
ms.custom: ''
ms.date: 09/12/2018
ms.technology:
- cpp-mfc
...
2.  Vyberte **Průvodce ovládacím prvkem MFC ActiveX** projekty v jazyce Visual C++ a pojmenujte svůj projekt.  
  
3.  Na **nastavení** stránce **asynchronně načte vlastnosti**. Výběrem této možnosti nastavíte vlastnost připraveném stavu a událostí změny stavu připraveno za vás.  
...
4.  Zvolte **Dokončit** pro vytvoření projektu.  
...
2.  V každém ze zdrojových souborů, které obsahuje soubor záhlaví ovládacího prvku přidejte hlavičkový soubor pro tuto třídu před ní.  
  
3.  V této třídě přepsat `OnDataAvailable`. Tato funkce je volána pokaždé, když se data jsou k dispozici k zobrazení. Jak budou data k dispozici, je možné ji zpracovávat žádným způsobem, který zvolíte, například postupně vykreslování.  
...
5.  Přepsat `OnProgress`. V `OnProgress`, jsou předány číslo zobrazující maximální rozsah a je číslo znázorňující, jak daleko podél aktuální soubor ke stažení. Tato čísla slouží k zobrazení stavu, jako je například procento dokončení uživateli.  
...
2.  V **Průvodce přidáním vlastnosti**, vyberte **Set/Get metody** přepínací tlačítko, zadejte **název vlastnosti**, například EditControlText a vyberte BSTR jako **Typ vlastnosti**.  
  
3.  Klikněte na tlačítko **Dokončit**.  
  
4.  Deklarujte proměnné člena vaší `CDataPathProperty`-odvozené třídy a třídy vašeho ovládacího prvku ActiveX.  
...
7.  Přepsat [ResetData](../mfc/reference/cdatapathproperty-class.md#resetdata) oznámit vlastnost resetování ovládacího prvku tak, že přidáte tento řádek:  
