---
title: Seznam ukázek dialogových oken
ms.date: 11/04/2016
helpviewer_keywords:
- sample applications [MFC], dialog boxes
ms.assetid: 3fc7dd7c-d758-4c43-96bb-0ea638ca1ad7
ms.openlocfilehash: 72470463c3d4ff402bca9c119456778cf7fadecb
ms.sourcegitcommit: 0ab61bc3d2b6cfbd52a16c6ab2b97a8ea1864f12
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 04/23/2019
ms.locfileid: "62384072"
---
# <a name="dialog-sample-list"></a>Seznam ukázek dialogových oken

Viz následující ukázkové programy, které ilustrují dialogová okna a seznamy vlastností:

*MDI ukázkovou aplikaci pomocí dialogových oknech*

- [SCRIBBLE](../overview/visual-cpp-samples.md)

*Nemodální dialogové okno*

- [NEMODÁLNÍ](../overview/visual-cpp-samples.md)

*Dialogové okno seznam vlastností (karta dialogové okno)*

- [PROPDLG](../overview/visual-cpp-samples.md)

- [CMNCTRL1](../overview/visual-cpp-samples.md)

- [CMNCTRL2](../overview/visual-cpp-samples.md)

*Aplikace založená na dialogové okno*

- [CMNCTRL1](../overview/visual-cpp-samples.md)

- [CMNCTRL2](../overview/visual-cpp-samples.md)

*Ovládací prvky dialogového okna*

- [CMNCTRL1](../overview/visual-cpp-samples.md)

- [CMNCTRL2](../overview/visual-cpp-samples.md)

- [CTRLTEST](../overview/visual-cpp-samples.md)

*Zobrazení formuláře jako dialogové okno*

- [VIEWEX](../overview/visual-cpp-samples.md)

*Šablony dialogového okna v paměti*

- [DLGTEMPL](../overview/visual-cpp-samples.md)

## <a name="see-also"></a>Viz také:

[Dialogová okna](../mfc/dialog-boxes.md)
