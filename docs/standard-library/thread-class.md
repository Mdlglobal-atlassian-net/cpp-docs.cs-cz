...
`F` Definované aplikací funkce být vykonán vlákno.

`A` Seznam argumentů, které mají být předány `F`.

`Other` Existující `thread` objektu.
