---
title: '&lt;type_traits&gt; | Dokumentace Microsoftu'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
...
ms.locfileid: "43218849"
---
# <a name="lttypetraitsgt"></a>&lt;type_traits&gt;

Definuje šablony, které poskytují konstanty z doby kompilace, které poskytují informace o vlastnostech jejich argumenty typu nebo mít transformuje typy.
...
Třídy a šablon v \<type_traits > se používá pro podporu odvození typu proměnné, klasifikace a transformace v době kompilace, ke zjištění chyby související s typem a které vám pomůžou optimalizovat obecný kód. Tyto třídy a šablony obsahují unární typové vlastnosti, které popisují vlastnosti typu binary typové vlastnosti, které popisují vztah mezi typy a vlastnosti transformace, které upravit vlastnost typu.

Pro podporu vlastností s typem, pomocná třída `integral_constant`, je definován. Má specializace šablony `true_type` a `false_type` , které tvoří základní třídy pro predikáty typu. A *predikát typu* je šablona, která přijímá jeden nebo více argumentů typu. Pokud predikát typu *platí*, veřejně odvozené, přímo nebo nepřímo ze [true_type](../standard-library/type-traits-typedefs.md#true_type). Pokud predikát typu *obsahuje hodnotu false*, veřejně odvozené, přímo nebo nepřímo ze [false_type](../standard-library/type-traits-typedefs.md#false_type).
...
|[has_virtual_destructor](https://msdn.microsoft.com/c0f85f0b-c63c-410d-a046-72eeaf44f7eb)|Ověřuje, zda typ má virtuální destruktor.|
...
|[result_of –](../standard-library/result-of-class.md)|Určuje návratový typ volatelného typu, který přijímá zadanými typy argumentu.|
