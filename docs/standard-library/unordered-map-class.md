...
`operator[]` slouží k vložení elementy do mapy *m* pomocí *m*[_ *klíč*] = `DataValue`, kde `DataValue` je hodnota `mapped_type` elementu s Hodnota klíče \_ *klíč*.
