---
title: unordered_map – třída | Dokumentace Microsoftu
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
...
Objekt seřadí sekvenci ovládá, voláním dvou uložených objektů: objektu funkce porovnání typu [unordered_map::key_equal](#key_equal) a objektu hashovací funkce typu [unordered_map::hasher](#hasher). Přístup k prvnímu uloženému objektu voláním členské funkce [unordered_map::key_eq](#key_eq)`()`; a přístup k druhému uloženému objektu voláním členské funkce [unordered_map::hash_function](#hash) `()`. Konkrétně pro všechny hodnoty `X` a `Y` typu `Key`, volání `key_eq()(X, Y)` vrátí true, pouze pokud hodnoty dvou argumentů mají stejné pořadí; volání `hash_function()(keyval)` získá distribuci hodnot typu `size_t`. Na rozdíl od třídy šablony [unordered_multimap – třída](../standard-library/unordered-multimap-class.md), objekt třídy šablony `unordered_map` zajišťuje, že `key_eq()(X, Y)` má vždy hodnotu false pro jakékoli dva prvky řízené sekvence. (Klíče jsou jedinečné).

Objekt také uchovává faktor maximálního zatížení, který určuje maximální požadovaný průměrný počet prvků na kbelík. Pokud vložení prvku způsobí, že [unordered_map::load_factor](#load_factor) `()` překročí faktor maximálního zatížení, kontejner zvýší počet kbelíků a podle potřeby znovu vytvoří tabulku hash.
...
Vyhledá prvek do objektu unordered_map se zadanou hodnotou klíče.
...
Pokud není nalezena klíčová hodnota argumentu, pak funkce vyvolá objekt třídy `out_of_range`.
...
První dvě členské funkce vrátí dopředný iterátor, který odkazuje na první prvek pořadí (nebo přesně za konec k prázdné sekvenci). Poslední dva členské funkce vrátí dopředný iterátor odkazující na první prvek kontejneru *nbucket* (nebo jenom za koncem prázdného kontejneru).
...
Členská funkce vrátí číslo kbelíku aktuálně odpovídající hodnotě klíče *keyval*.
...
Členská funkce vrátí aktuální počet kbelíků.
...
Členské funkce vrátí velikost položky číslo kbelíku *nbucket*.
...
A **const** iterátor pro dopředný přístup, který ukazuje přesně za konec rozsahu.
...
Typ popisuje objekt, který může sloužit jako konstantní dopředného iterátoru řízené sekvence. Je popsán jako synonymum pro implementací definovaný typ. `T1`.
...
Typ popisuje objekt, který může sloužit jako konstantní dopředný iterátor, který pro blok. Je popsán jako synonymum pro implementací definovaný typ. `T5`.
...
Členská funkce vrátí počet prvků v rozsahu odděleny [unordered_map::equal_range](#equal_range)`(keyval)`.
...
Typ celé číslo se znaménkem, který popisuje objekt, který může představovat rozdíl mezi adresami dva prvky řízené sekvence. Je popsán jako synonymum pro implementací definovaný typ. `T3`.
...
Vloží vytvořený prvek na místě (jsou prováděny žádné operace kopírování nebo přesunutí) do objektu unordered_map.
...
|Parametr|Popis|
|-|-|
|*argumenty*|Argumenty předané vytvořit element, který má být vložen do objektu unordered_map, pokud již obsahuje prvek, jehož hodnota je ekvivalentně seřazen.|

### <a name="return-value"></a>Návratová hodnota

A `pair` jehož **bool** komponenty vrátí hodnotu true, pokud bylo vložení provedeno a false v případě `unordered_map` již obsahuje prvek, jehož klíč má ekvivalentní hodnotu v pořadí, a jehož komponenta iterátoru vrátí řešení, ve kterém byl vložen nový element nebo element se kdy již nachází.

Chcete-li přistupovat ke komponentě iterátoru dvojice `pr` vrácený tato členská funkce, použijte `pr.first`a můžete přistoupit přes ukazatel, pomocí `*(pr.first)`. Pro přístup **bool** součásti páru `pr` vrácený tato členská funkce, použijte `pr.second`.

### <a name="remarks"></a>Poznámky

Touto funkcí nejsou zneplatněny žádné iterátory nebo odkazy.

Při vkládání Pokud je vyvolána výjimka, ale nenastane v kontejneru funkce hash, kontejneru se nezmění. Pokud je vyvolána výjimka ve funkci hash, výsledek není definován.
...
|Parametr|Popis|
|-|-|
|*argumenty*|Argumenty předané vytvořit element, který má být vložen do objektu unordered_map, pokud objekt unordered_map již obsahuje tento prvek nebo obecně platí, pokud ho již obsahuje prvek, jehož klíč je ekvivalentně seřazen.|
...
Touto funkcí nejsou zneplatněny žádné odkazy.
...
První dvě členské funkce vrátí dopředný iterátor, který ukazuje za konec sekvence. Poslední dva členské funkce vrátí dopředný iterátor, který ukazuje za konec bloku *nbucket*.
...
Členská funkce vrátí pár iterátorů `X` tak, aby `[X.first, X.second)` vymezuje pouze tyto prvky řízené sekvence, která mají stejné pořadí s *keyval*. Pokud neexistuje žádný takový prvek obou iterátory jsou `end()`.
...
Odebere prvek nebo rozsah prvků do objektu unordered_map od zadané pozice nebo odebere prvky, které odpovídají zadanému klíči.
...
Třetí členská funkce vrátí počet prvků, které byly odebrány z objektu unordered_map.
...
Členská funkce vrátí uložený objekt hashovací funkce.
...
|*ValTy*|Parametr šablony určující typ argumentu, který objekt unordered_map může použít k vytvoření prvku [value_type](../standard-library/map-class.md#value_type)a dokonalému předání *Val* jako argument.|
...
Jedním prvkem členské funkce (1) a (2) vrátí [pár](../standard-library/pair-structure.md) jehož **bool** komponenta je hodnota true, pokud bylo vložení provedeno a hodnotu false, pokud objekt unordered_map již obsahuje prvek, jehož klíč má ekvivalentní hodnotu v pořadí. Komponenta iterátoru dvojice návratové hodnoty odkazuje na nově vložený prvek, pokud **bool** komponenta je nastavena hodnota true, nebo do existujícího prvku Pokud **bool** komponenta má hodnotu false.

Jeden element s nápovědu členské funkce, (3) a (4) vrátí iterátor odkazující na pozici, kde byl vložen nový prvek do objektu unordered_map, nebo pokud prvek s ekvivalentním klíčem již existuje, do existujícího prvku.
...
[Value_type](../standard-library/map-class.md#value_type) kontejneru je definice typu, který patří do tohoto kontejneru a pro mapu, `map<K, V>::value_type` je `pair<const K, V>`. Hodnota prvku je seřazená dvojice, ve které je první komponenta rovna hodnotě klíče a druhá komponenta je rovna datové hodnotě prvku.

Rozsah členské funkce (5) vloží sekvenci hodnot prvků do objektu unordered_map, který odpovídá každému prvku určenému pomocí iterátoru v rozsahu `[First, Last)`; proto `Last` nebude vložen. Členská funkce kontejneru `end()` se vztahuje k pozici hned za posledním prvkem v kontejneru, například příkaz `m.insert(v.begin(), v.end());` se pokusí vložit všechny prvky `v` do `m`. Vkládají se pouze prvky, které v rozsahu obsahují jedinečné hodnoty. Duplicitní hodnoty jsou ignorovány. Chcete-li sledovat, které prvky jsou odmítnuty, použijte jednoprvkovou verzi funkce `insert`.

Funkce člena seznamu inicializátorů (6) používá [initializer_list](../standard-library/initializer-list.md) pro kopírování prvků do objektu unordered_map.

Pro vložení prvku vytvořeného na místě – to znamená, jsou prováděny žádné operace kopírování nebo přesunutí – naleznete v tématu [unordered_map::emplace](#emplace) a [unordered_map::emplace_hint](#emplace_hint).
...
Typ popisuje objekt, který může sloužit jako dopředného iterátoru řízené sekvence. Je popsán jako synonymum pro implementací definovaný typ. `T0`.
...
Členská funkce vrátí `(float)` [unordered_map::size](#size)`() / (float)`[unordered_map::bucket_count](#bucket_count)`()`, průměrný počet prvků na kbelík.
...
Typ popisuje objekt, který může sloužit jako dopředný iterátor, který pro blok. Je popsán jako synonymum pro implementací definovaný typ. `T4`.
...
Členská funkce vrátí maximální počet kbelíků aktuálně povoleno.
...
První členská funkce vrátí faktor maximálního zatížení uložené. Druhá členská funkce se nahradí faktor maximálního zatížení uložené s *faktor*.
...
`operator[]` lze vložit prvky do mapy *m* pomocí *m*[_ *klíč*] = `DataValue`, kde `DataValue` je hodnota `mapped_type` elementu, který se klíč hodnoty \_ *klíč*.

Při použití `operator[]` vložit prvky, vrácený odkaz neudává, zda je vložení změna již existující element nebo vytvoří nový. Členské funkce [najít](../standard-library/map-class.md#find) a [vložit](../standard-library/map-class.md#insert) slouží k určení, zda element s určeným klíčem již existuje před vložením.
...
Členská funkce určuje iterátor `where` jako návratovou hodnotu [unordered_map::insert](#insert) `(` [unordered_map::value_type](#value_type)`(keyval, Ty())`. (Pokud neexistuje žádný takový prvek, vloží prvek se zadaným klíčem.) Vrátí odkaz na `(*where).second`.

## <a name="op_eq"></a>  unordered_map::Operator =

Nahradí prvky objektu tohoto objektu unordered_map, konkrétně pomocí elementů z jiného objektu unordered_map.
...
|Parametr|Popis|
|-|-|
|*doprava*|Unordered_map, který přiřazuje obsah z funkce operátoru.|

### <a name="remarks"></a>Poznámky

První verze zkopíruje všechny prvky z *správné* do tohoto objektu unordered_map.

Druhá verze přesune všechny prvky z *správné* do tohoto objektu unordered_map.

Všechny prvky, které jsou v tomto unordered_map před `operator`= spustí se zahodí.
...
Členská funkce mění počet kbelíků nejméně *nbuckets* a znovu vytvoří hashovací tabulku podle potřeby.
...
Členská funkce vrátí délku objektu řízené sekvence.
...
Typ celé číslo bez znaménka, který popisuje objekt, který může představovat délka jakékoli řízené sekvence. Je popsán jako synonymum pro implementací definovaný typ. `T2`.
...
Členská funkce Zamění řízené sekvence mezi `*this` a *správné*. Pokud [unordered_map::get_allocator](#get_allocator)`() == right.get_allocator()`, provádí se v konstantním času a vyvolá výjimku pouze v důsledku kopírování uložené vlastnosti objektu typu `Tr`, a to nezruší platnost žádné odkazy, ukazatele, nebo iterátory, které určují prvky v dané dvě řízené sekvence. V opačném případě provede několik element přiřazení a volání konstruktoru je přímo úměrný počtu prvků v dané dvě řízené sekvence.
...
První konstruktor určuje kopii sekvence řízenou parametrem `right`. Druhý konstruktor určuje prázdnou řízenou sekvenci. Třetí konstruktor vloží sekvenci hodnot prvku `[first, last)`. Čtvrtý konstruktor určuje kopii sekvence posunutím `right`.
...
Objekt funkce porovnání je argument *kompozici*, pokud existuje; v opačném případě je `Pred()`.
...
Typ, který popisuje element řízené sekvence.
