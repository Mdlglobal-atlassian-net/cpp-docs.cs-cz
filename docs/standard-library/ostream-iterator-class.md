...
*Typ* typ objektu, který má být vložena do výstupního datového proudu.

`CharType` Typ, který představuje typ znaku pro `ostream_iterator`. Tento argument je volitelný a výchozí hodnota je `char`.

`Traits` Typ, který představuje typ znaku pro `ostream_iterator`. Tento argument je volitelný a výchozí hodnota je `char_traits` \< *CharType >.*
...
|[operátor =](#op_eq)|Operátor přiřazení použít k implementaci výraz iterator výstup * `i`  =  `x` k zápisu do výstupního proudu.|
...
`val` Hodnota typu objektu `Type` má být vložena do výstupního datového proudu.
...
`_Ostr` Výstupní datový proud typu [ostream_iterator::ostream_type](#ostream_type) k být vstupní přes.

`_Delimiter` Oddělovač, který je vložen do výstupního datového proudu mezi hodnotami.
