...
Šablona třídy popisuje distribuci, která vytváří hodnoty typu s plovoucí desetinnou čárkou, nebo typ **Double** , pokud není k dispozici, distribuované podle rozdělení v fisherích. Následující tabulka obsahuje odkazy na články týkající se jednotlivých členů.
...
Funkce vlastností `m()` a `n()` vrací hodnoty pro uložené distribuční parametry `m` a `n`.
...
První konstruktor vytvoří objekt, jehož uložená `m` hodnota obsahuje hodnotu *m* a jejíž uložená `n` hodnota obsahuje hodnotu *n*.
