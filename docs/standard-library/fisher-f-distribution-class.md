...
ms.locfileid: "81319600"
---
# <a name="fisher_f_distribution-class"></a>fisher_f_distribution – třída
...
Šablona třídy popisuje rozdělení, které vytváří hodnoty uživatelem určeného typu s plovoucí desetinnou čárkou, nebo zadejte **double,** pokud není k dispozici žádná, distribuovaná podle Fisherova F-Distribution. Následující tabulka odkazuje na články o jednotlivých členech.
...
Vlastnost funkce `m()` `n()` a vrátit hodnoty pro `m` parametry `n` uložené distribuce a příslušně.
...
Podrobné informace o f-distribuci naleznete v článku [F-Distribution](https://go.microsoft.com/fwlink/p/?linkid=400899)ve Wolfram MathWorld .
...
**Předpokladem:** `0.0 < m` a`0.0 < n`

První konstruktor zkonstruoval objekt, jehož `m` uložená hodnota obsahuje hodnotu *m* a jehož uložená `n` hodnota obsahuje hodnotu *n*.
