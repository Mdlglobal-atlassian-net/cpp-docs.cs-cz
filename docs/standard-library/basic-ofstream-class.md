...
`right` Deklarátor odkazu na `basic_ofstream` objekt používaný k inicializaci to `basic_ofstream` objektu.
...
`right` Rvalue odkaz na `basic_ofstream` objektu.
...
`right` `lvalue` Odkaz na jiný `basic_ofstream` objektu.
