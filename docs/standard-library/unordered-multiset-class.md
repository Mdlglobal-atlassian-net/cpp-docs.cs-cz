---
title: unordered_multiset – třída | Dokumentace Microsoftu
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
...
Objekt seřadí sekvenci ovládá, voláním dvou uložených objektů: objektu funkce porovnání typu [unordered_multiset::key_equal](#key_equal) a objektu hashovací funkce typu [unordered_multiset::hasher](#hasher). Přístup k prvnímu uloženému objektu voláním členské funkce [unordered_multiset::key_eq](#key_eq)`()`; a přístup k druhému uloženému objektu voláním členské funkce [unordered_multiset::hash_ funkce](#hash)`()`. Konkrétně pro všechny hodnoty `X` a `Y` typu `Key`, volání `key_eq()(X, Y)` vrátí true, pouze pokud hodnoty dvou argumentů mají stejné pořadí; volání `hash_function()(keyval)` získá distribuci hodnot typu `size_t`. Na rozdíl od třídy šablony [unordered_set – třída](../standard-library/unordered-set-class.md), objekt třídy šablony `unordered_multiset` nezajistí, že `key_eq()(X, Y)` má vždy hodnotu false pro jakékoli dva prvky řízené sekvence. (Klíče nemusí být jedinečné.)

Objekt také uchovává faktor maximálního zatížení, který určuje maximální požadovaný průměrný počet prvků na kbelík. Pokud vložení prvku způsobí, že [unordered_multiset::load_factor](#load_factor) `()` překročí faktor maximálního zatížení, kontejner zvýší počet kbelíků a podle potřeby znovu vytvoří tabulku hash.
...
Členská funkce vrátí počet prvků v rozsahu odděleny [unordered_multiset::equal_range](#equal_range)`(keyval)`.
...
|Parametr|Popis|
|-|-|
|*argumenty*|Argumenty předané vytvořit element, který má být vložen do unordered_multiset.|
...
Pro první dvě členské funkce obousměrný iterátor, který označí první prvek zbývající za jakýmikoli odstraněnými prvky, nebo element, který je koncem objektu unordered_multiset, pokud žádný takový prvek neexistuje.
...
|Parametr|Popis|
|-|-|
|*Val*|Hodnota element, který má být vložen do unordered_multiset.|
|*kde*|Místo zahájení vyhledání správného bodu vložení.|
|*ValTy*|Parametr šablony určující typ argumentu, unordered_multiset – můžete použít k vytvoření prvku [value_type](../standard-library/map-class.md#value_type)a dokonalému předání *Val* jako argument.|
...
Jeden element vložení členských funkcí (1) a (2) vrátí iterátor na místo, kde byl vložen nový prvek do unordered_multiset –.

Jeden element s nápovědu členské funkce, (3) a (4) vrátí iterátor, který odkazuje na místo, kde byl vložen nový prvek do unordered_multiset –.
...
[Value_type](../standard-library/map-class.md#value_type) kontejneru je definice typu, který patří do tohoto kontejneru a sady pro `unordered_multiset<V>::value_type` je typ `const V`.

Rozsah členské funkce (5) vloží sekvenci hodnot prvků do unordered_multiset, který odpovídá každému prvku určenému pomocí iterátoru v rozsahu `[First, Last)`; proto *poslední* nebude vložen. Členská funkce kontejneru `end()` vztahuje k pozici hned za posledním prvkem v kontejneru, například příkaz `m.insert(v.begin(), v.end());` vloží všechny prvky `v` do `m`.

Funkce člena seznamu inicializátorů (6) používá [initializer_list](../standard-library/initializer-list.md) pro kopírování prvků do unordered_multiset.

Pro vložení prvku vytvořeného na místě – to znamená, jsou prováděny žádné operace kopírování nebo přesunutí – naleznete v tématu [unordered_multiset::emplace](#emplace) a [unordered_multiset::emplace_hint](#emplace_hint).
...
Typ, který poskytuje konstantní [dopředný iterátor, který](../standard-library/forward-iterator-tag-struct.md) , který může číst prvky unordered_multiset.
...
Podívejte se na příklad pro [začít](../standard-library/multiset-class.md#begin) příklad toho, jak deklarace a používání **iterátoru**.
...
Členská funkce vrátí `(float)` [unordered_multiset::size](#size)`() / (float)`[unordered_multiset::bucket_count](#bucket_count)`()`, průměrný počet prvků na kbelík.
...
|Parametr|Popis|
|-|-|
|*doprava*|[Unordered_multiset](../standard-library/unordered-multiset-class.md) kopírovaná do `unordered_multiset`.|

### <a name="remarks"></a>Poznámky

Po odstranění jakýchkoli prvků v `unordered_multiset`, `operator=` kopíruje nebo přesouvá obsah *správné* do `unordered_multiset`.
...
První konstruktor určuje kopii sekvence řízenou parametrem *vpravo*. Druhý konstruktor určuje prázdnou řízenou sekvenci. Třetí konstruktor vloží sekvenci hodnot prvku `[First, Last)`. Čtvrtý konstruktor určuje kopii sekvence posunutím *vpravo*.
