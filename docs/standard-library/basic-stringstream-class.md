...
Šablona třídy popisuje objekt, který ovládá vkládání a extrakci prvků a kódovaných objektů pomocí vyrovnávací paměti datového proudu třídy [basic_stringbuf](../standard-library/basic-stringbuf-class.md) < **elem**, **TR**, `Alloc` >, s prvky typu `Elem`, jejichž vlastnosti znaků jsou určeny třídou `Tr` a jejichž prvky jsou přiděleny přidělováním `Alloc` třídy. Objekt ukládá objekt třídy basic_stringbuf < **elem**, **Tr**, `Alloc` >.
