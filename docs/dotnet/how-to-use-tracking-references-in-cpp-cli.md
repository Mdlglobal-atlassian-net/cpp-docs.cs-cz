---
title: 'Postupy: používání sledovacích odkazů v jazyce C + +/ CLI | Dokumentace Microsoftu'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
...
Další příklad ukazuje tento přebírání adresy odkazu sledování vrátí [interior_ptr (C + +/ CLI)](../windows/interior-ptr-cpp-cli.md)a ukazuje, jak upravit a přístup k datům prostřednictvím odkazem sledování.
...
Následující příklad kódu ukazuje, že je možné převádět mezi sledovacích odkazů a vnitřních ukazatelů.
...
Další příklad ukazuje, můžete mít sledovacích odkazů a nativní odkazy na typy hodnot.
...
Následující příklad ukazuje, které můžete použít sledovací odkazy spolu s typy hodnot a nativní typy.
...
Tento příklad ukazuje, že můžete vytvořit vazbu sledovací odkaz na typ hodnoty na haldě uvolňování:
...
## <a name="template-functions-that-take-native-value-or-reference-parameters"></a>Šablony funkcí, které přijímají nativní, hodnoty nebo parametry odkazů

S použitím sledovací odkaz v signatuře funkce šablony, zajistíte tím, že funkce je možné vyvolat v parametr, jehož typ je nativní, CLR hodnoty nebo odkazu modulu CLR.
