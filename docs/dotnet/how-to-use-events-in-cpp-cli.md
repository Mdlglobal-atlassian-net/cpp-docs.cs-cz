---
title: 'Postupy: používání událostí v jazyce C + +/ CLI | Microsoft Docs'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
...
## <a name="interface-events"></a>Události rozhraní  
 Následující příklad kódu přidá obslužnou rutinu události, vyvolá událost, což způsobí, že obslužné rutiny události pro zápis její název do konzoly – a poté odstraní obslužné rutiny události.  
...
## <a name="custom-accessor-methods"></a>Vlastní přístupových metod vlastností  
 Následující příklad ukazuje, jak definovat chování události při přidávání nebo odebírání obslužné rutiny, a když se vyvolá událost.  
...
## <a name="override-default-access-on-add-remove-and-raise-accessors"></a>Přepsání výchozího přístupu na přidání, odebrání a vyvolání přístupové objekty  
 Tato ukázka ukazuje, jak přepsat výchozí úroveň přístupu přidat, odebrat a vyvolat události metody:  
...
## <a name="multiple-event-handlers"></a>Více obslužných rutin událostí  
 Přijímače událostí nebo jakýkoli jiný kód klienta, můžete přidat jeden nebo více obslužné rutiny události.  
...
## <a name="static-events"></a>Statické události  
 Následující příklad ukazuje, jak k definování a používání statických událostí.  
...
## <a name="virtual-events"></a>Virtuálních událostí  
 Tato ukázka implementuje virtuální, kterou spravuje události do rozhraní a třídy:  
...
```  
 ...
 Jednoduchá událost nelze zadat přepsání nebo skryjete události základní třídy.  Musí definovat všechny funkce přístupového objektu události a poté zadejte `new` nebo `override` – klíčové slovo na jednotlivé funkce přistupujícího objektu.  
...
## <a name="raising-events-that-are-defined-in-a-different-assembly"></a>Vyvolání událostí, které jsou definovány v jiném sestavení  
 Událostí a obslužné rutiny události může být definované v jednom sestavení a spotřebovávají jiné sestavení.  
...
```  
 ...
 Tento kód klienta zpracovává událost:  
