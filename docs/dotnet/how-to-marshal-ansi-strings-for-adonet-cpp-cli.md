---
title: 'Postupy: Zařazování řetězců ANSI pro technologii ADO.NET (C + +/ CLI) | Microsoft Docs'
...
ms.locfileid: "33137468"
---
# <a name="how-to-marshal-ansi-strings-for-adonet-ccli"></a>Postupy: Zařazování řetězců ANSI pro technologii ADO.NET (C++/CLI)
Ukazuje, jak přidat nativní řetězec (`char *`) k databázi a jak přeuspořádat <xref:System.String?displayProperty=fullName> z databáze na nativní řetězec.  
...
## <a name="example"></a>Příklad  
 ...
 Zbytek kód v tomto příkladu je nativní kód C++, jako je indikován `#pragma unmanaged` předchází `main`. V tomto příkladu jsme se vytváření novou instanci třídy DatabaseClass a volání její metody pro vytvoření tabulky a naplnit některé řádky v tabulce. Všimněte si, že nativní řetězce C++ jsou předávány jako hodnoty pro sloupec databáze StringCol. Uvnitř DatabaseClass, tyto řetězce přeuspořádány na spravované řetězce použití funkcionality v nalezen <xref:System.Runtime.InteropServices?displayProperty=fullName> oboru názvů. Konkrétně metoda <xref:System.Runtime.InteropServices.Marshal.PtrToStringAnsi%2A> se používá k zařazování `char *` k <xref:System.String>a metodu <xref:System.Runtime.InteropServices.Marshal.StringToHGlobalAnsi%2A> se používá k zařazování <xref:System.String> k `char *`.  
  
> [!NOTE]
>  Paměti přidělené <xref:System.Runtime.InteropServices.Marshal.StringToHGlobalAnsi%2A> musí být navrácena voláním buď <xref:System.Runtime.InteropServices.Marshal.FreeHGlobal%2A> nebo `GlobalFree`.  
