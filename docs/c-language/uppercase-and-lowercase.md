---
title: Velká a malá písmena | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-language
ms.topic: language-reference
dev_langs:
- C++
ms.assetid: f33caa65-15ce-4b06-97a6-d19b3ab4d5ad
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
ms.openlocfilehash: 22eade414a42913d3fbd3c5281336279af6390cb
ms.sourcegitcommit: be2a7679c2bd80968204dee03d13ca961eaa31ff
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 05/03/2018
---
# <a name="uppercase-and-lowercase"></a>Velká a malá písmena
**ANSI 3.1.2** jestli případu rozdíly jsou významné  
  
 Microsoft C zpracovává identifikátory v rámci kompilace jednotky jako malá a velká písmena.  
  
 Microsoft linkeru je malá a velká písmena. Je nutné zadat všechny identifikátory konzistentně podle případu.  
  
## <a name="see-also"></a>Viz také  
 [Chování identifikátorů](../c-language/behavior-of-identifiers.md)