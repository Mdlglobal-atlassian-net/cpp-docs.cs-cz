...
1. Použijte **Průvodce aplikací knihovny MFC** k vytvoření aplikace MFC se všemi výchozími nastaveními. Pokyny k otevření Průvodce pro vaši verzi sady Visual Studio najdete v tématu [Návod: použití nových ovládacích prvků prostředí MFC](../../mfc/walkthrough-using-the-new-mfc-shell-controls.md) .

1. Zadejte název projektu, například `Mandelbrot`a potom kliknutím na tlačítko **OK** zobrazte **Průvodce aplikací knihovny MFC**.
...
1. V souboru *PCH. h* (*stdafx. h* v aplikaci Visual Studio 2017 a starší) přidejte následující direktivu `#include`:

   [!code-cpp[concrt-mandelbrot#1](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_1.h)]

1. V ChildView. h po direktivě `pragma` Definujte typ `BitmapPtr`. Typ `BitmapPtr` umožňuje, aby se ukazatel na objekt `Bitmap` sdílel více komponentami. Objekt `Bitmap` je odstraněn, když na něj již neodkazuje žádná součást.

   [!code-cpp[concrt-mandelbrot#2](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_2.h)]

1. V ChildView. h přidejte následující kód do oddílu `protected` `CChildView` třídy:
...
1. V ChildView. cpp, odkomentujte nebo odeberte následující řádky.
   ...
   V ladicích sestaveních tento krok zabrání aplikaci v použití modulu přidělování `DEBUG_NEW`, který je nekompatibilní s rozhraním GDI+.

1. V ChildView. cpp přidejte direktivu `using` do oboru názvů `Gdiplus`.

   [!code-cpp[concrt-mandelbrot#5](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_5.cpp)]

1. Přidejte následující kód do konstruktoru a destruktoru třídy `CChildView` pro inicializaci a vypnutí rozhraní GDI+.

   [!code-cpp[concrt-mandelbrot#6](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_6.cpp)]

1. Implementujte metodu `CChildView::DrawMandelbrot`. Tato metoda nakreslí Mandelbrot Fractal na zadaný objekt `Bitmap`.

   [!code-cpp[concrt-mandelbrot#7](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_7.cpp)]

1. Implementujte metodu `CChildView::OnPaint`. Tato metoda volá `CChildView::DrawMandelbrot` a potom zkopíruje obsah objektu `Bitmap` do okna.
...
Tento příklad také používá objekt [Concurrency:: unbounded_buffer](reference/unbounded-buffer-class.md) , který umožňuje VLÁKNU uživatelského rozhraní a pracovnímu vláknu vzájemně komunikovat. Poté, co pracovní vlákno vytvoří obrázek, pošle ukazatel na objekt `Bitmap` do objektu `unbounded_buffer` a pak odešle zprávu o Malování do vlákna uživatelského rozhraní. Vlákno UI pak získá z objektu `unbounded_buffer` objekt `Bitmap` a nakreslí ho do okna klienta.

#### <a name="to-remove-the-drawing-work-from-the-ui-thread"></a>Odebrání práce kreslení z vlákna uživatelského rozhraní

1. V souboru *PCH. h* (*stdafx. h* v aplikaci Visual Studio 2017 a starší) přidejte následující direktivy `#include`:

   [!code-cpp[concrt-mandelbrot#101](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_9.h)]

1. V ChildView. h přidejte členské proměnné `task_group` a `unbounded_buffer` do oddílu `protected` třídy `CChildView`. Objekt `task_group` obsahuje úlohy, které provádějí vykreslování; objekt `unbounded_buffer` obsahuje dokončenou bitovou kopii Mandelbrot.

   [!code-cpp[concrt-mandelbrot#102](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_10.h)]

1. V ChildView. cpp přidejte direktivu `using` do oboru názvů `concurrency`.

   [!code-cpp[concrt-mandelbrot#103](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_11.cpp)]

1. V metodě `CChildView::DrawMandelbrot` po volání `Bitmap::UnlockBits`zavolejte funkci [Concurrency:: Send](reference/concurrency-namespace-functions.md#send) pro předání objektu `Bitmap` VLÁKNU uživatelského rozhraní. Pak odešlete zprávu o Malování do vlákna uživatelského rozhraní a zrušte ověření klientské oblasti.

   [!code-cpp[concrt-mandelbrot#104](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_12.cpp)]

1. Aktualizujte metodu `CChildView::OnPaint` pro příjem aktualizovaného objektu `Bitmap` a nakreslete image do okna klienta.

   [!code-cpp[concrt-mandelbrot#105](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_13.cpp)]

   Metoda `CChildView::OnPaint` vytvoří úlohu pro vygenerování image Mandelbrot, pokud jedna neexistuje ve vyrovnávací paměti zpráv. Vyrovnávací paměť zprávy nebude obsahovat objekt `Bitmap` v případech, jako je například zpráva o počátečním Malování a při přesunu jiného okna před oknem klienta.
...
Generování Mandelbrot Fractal je dobrým kandidátem na paralelismus, protože výpočet každého pixelu je nezávislý na všech ostatních výpočtech. Chcete-li paralelizovat postup kreslení, převeďte vnější `for` smyčka v metodě `CChildView::DrawMandelbrot` na volání algoritmu [Concurrency::p arallel_for](reference/concurrency-namespace-functions.md#parallel_for) následujícím způsobem.
...
Zrušení dokumentu [ve PPL](cancellation-in-the-ppl.md) vysvětluje, jak zrušení funguje v modulu runtime. Zrušení je kooperativní; proto k němu nedochází hned. Chcete-li zastavit zrušený úkol, modul runtime vyvolá vnitřní výjimku při následném volání z úlohy do modulu runtime. V předchozí části se dozvíte, jak použít algoritmus `parallel_for` ke zlepšení výkonu úlohy kreslení. Volání `parallel_for` umožňuje modulu runtime zastavit úlohu, a proto umožňuje zrušení práce.

### <a name="cancelling-active-tasks"></a>Rušení aktivních úloh

Aplikace Mandelbrot vytvoří objekty `Bitmap`, jejichž rozměry odpovídají velikosti okna klienta. Pokaždé, když se změní velikost okna klienta, aplikace vytvoří další úlohu na pozadí, která vygeneruje obrázek pro nové velikosti okna. Aplikace nevyžaduje tyto mezilehlé image. vyžaduje pouze obrázek pro konečnou velikost okna. Chcete-li aplikaci zabránit v provádění této další práce, můžete zrušit všechny aktivní úlohy kreslení v obslužných rutinách zpráv pro `WM_SIZE` a `WM_SIZING` zprávy a následně znovu naplánovat práci při kreslení v případě, že dojde ke změně velikosti okna.

Chcete-li zrušit aktivní úlohy kreslení při změně velikosti okna, aplikace zavolá metodu [Concurrency:: task_group:: Cancel](reference/task-group-class.md#cancel) v obslužných rutinách pro zprávy `WM_SIZING` a `WM_SIZE`. Obslužná rutina zprávy `WM_SIZE` také volá metodu [Concurrency:: task_group:: wait](reference/task-group-class.md#wait) , aby čekala na dokončení všech aktivních úloh, a poté znovu naplánovala úlohu kreslení pro aktualizovanou velikost okna.

Po zničení okna klienta je vhodné zrušit všechny aktivní úlohy kreslení. Zrušením aktivních úloh kreslení se zajistí, že pracovní vlákna po zničení okna klienta neodesílají zprávy do vlákna uživatelského rozhraní. Aplikace zruší všechny aktivní úlohy kreslení v obslužné rutině zprávy `WM_DESTROY`.

### <a name="responding-to-cancellation"></a>Reakce na zrušení

Metoda `CChildView::DrawMandelbrot`, která provádí úlohu kreslení, musí reagovat na zrušení. Vzhledem k tomu, že modul runtime používá ke zrušení úloh zpracování výjimek, musí metoda `CChildView::DrawMandelbrot` používat mechanismus bezpečný pro výjimku, aby bylo zaručeno, že všechny prostředky budou správně vyčištěny. V tomto příkladu se používá vzor pro *získání prostředků* (RAII), který zaručuje, že při zrušení úlohy dojde k odemčení bitů rastrového obrázku.

##### <a name="to-add-support-for-cancellation-in-the-mandelbrot-application"></a>Přidání podpory pro zrušení v aplikaci Mandelbrot

1. V ChildView. h v sekci `protected` třídy `CChildView` přidejte deklarace pro funkce map zpráv `OnSize`, `OnSizing`a `OnDestroy`.

   [!code-cpp[concrt-mandelbrot#201](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_15.h)]

1. V ChildView. cpp Upravte mapu zprávy tak, aby obsahovala obslužné rutiny pro zprávy `WM_SIZE`, `WM_SIZING`a `WM_DESTROY`.

   [!code-cpp[concrt-mandelbrot#202](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_16.cpp)]

1. Implementujte metodu `CChildView::OnSizing`. Tato metoda zruší všechny existující úlohy kreslení.

   [!code-cpp[concrt-mandelbrot#203](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_17.cpp)]

1. Implementujte metodu `CChildView::OnSize`. Tato metoda zruší všechny existující úlohy kreslení a vytvoří novou úlohu kreslení pro aktualizovanou velikost okna klienta.

   [!code-cpp[concrt-mandelbrot#204](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_18.cpp)]

1. Implementujte metodu `CChildView::OnDestroy`. Tato metoda zruší všechny existující úlohy kreslení.
...
1. Po volání `Bitmap::LockBits`přidejte následující kód do metody `CChildView::DrawMandelbrot`:

   [!code-cpp[concrt-mandelbrot#207](../../parallel/concrt/codesnippet/cpp/walkthrough-removing-work-from-a-user-interface-thread_21.cpp)]

   Tento kód zpracovává zrušení vytvořením objektu `scope_guard`. Když objekt opustí rozsah, odemkne rastrové bity.

1. Po odemčení rastrových obrázků, ale před odesláním všech zpráv do vlákna uživatelského rozhraní, upravte konec metody `CChildView::DrawMandelbrot`, aby bylo možné objekt `scope_guard` zavřít. Tím se zajistí, že před odemknutím rastrových obrázků není vlákno uživatelského rozhraní aktualizováno.
