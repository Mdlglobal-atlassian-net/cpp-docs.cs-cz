...
#### <a name="cancellation-tokens-and-task-composition"></a>Tokeny zrušení a skládání úloh  
 ...
 [Souběžnosti:: when_all HYPERLINK "http://msdn.microsoft.com/library/system.threading.tasks.task.whenall (v=VS.110).aspx" –](reference/concurrency-namespace-functions.md#when_all) a [concurrency::when_any](reference/concurrency-namespace-functions.md#when_all) funkce vám může pomoct vytvořit více úloh k implementaci běžných vzorů. Tato část popisuje, jak tyto funkce fungují s zrušení tokenů.  
