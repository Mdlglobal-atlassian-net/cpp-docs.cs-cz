...
> [!NOTE]
>  Podobné informace z hlediska knihovny MFC naleznete v tématu [Multithreading: Programovací tipy](../parallel/multithreading-programming-tips.md) a [Multithreading: kdy použít synchronizační třídy](../parallel/multithreading-when-to-use-the-synchronization-classes.md).  
...
Win32 také poskytuje úložiště Thread Local (TLS) k ukládání dat pro vlákno. Další informace najdete v tématu [vlákna místní úložiště (TLS)](../parallel/thread-local-storage-tls.md).  
