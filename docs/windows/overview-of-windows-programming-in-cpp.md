...
Existuje několik kategorií aplikací Windows, které můžete vytvořit pomocí jazyka C++. Každý má své vlastní programovací model a sadu knihoven, specifické pro Windows, ale C++ třetích stran a standardní knihovny C++ knihovny můžete použít v některé z nich.
...
A *nativní klientská aplikace* je C nebo C++ oddílové aplikaci, která používá původní nativní [modelu COM (Component Object) rozhraní API ani rozhraní API Windows C](/windows/desktop/apiindex/windows-api-list) pro přístup k operačním systémem. Tato rozhraní API představují samy o sobě převážně v C. Existuje více než jeden způsob, jak vytvořit nativní aplikaci klasické pracovní plochy: Můžete naprogramovat pomocí rozhraní API Win32 přímo, pomocí smyčky zpráv ve stylu jazyka C, který zpracovává události v operačním systému. Nebo můžete naprogramovat pomocí *Microsoft Foundation Classes* (MFC) lehce objektově orientované C++ knihovnu, která zabalí Win32. Ani přístup se považuje za "moderní" porovnání pro univerzální platformu Windows (UPW), ale oba jsou stále plně podporovány a mít miliony řádků kódu spuštěného v celém světě ještě dnes. Aplikace Win32, na kterém běží v okně vyžaduje pro vývojáře pro práci s zpráv Windows uvnitř procedury funkce Windows explicitně. Bez ohledu na název můžete jako (x86) 32bitové nebo 64bitové (x64) binární zkompilovat aplikaci Win32. V sadě Visual Studio IDE je shodný podmínky x86 a Win32.
...
Je také možné kombinovat výkon a efektivitu C++ s .NET – programování. Můžete vytvořit uživatelské rozhraní v C# a používat C++vyhodnocovací, aby aplikace využívat nativní C++ knihovny. Další informace najdete v tématu [programování v rozhraní .NET s C++vyhodnocovací](../dotnet/dotnet-programming-with-cpp-cli-visual-cpp.md).

## <a name="com-components"></a>Komponenty modelu COM

[Modelu COM (Component Object)](/windows/desktop/com/the-component-object-model) je specifikace, které umožňuje programy napsané v různých jazycích, aby mezi sebou komunikovat. Řada komponent Windows jsou implementovány jako objekty modelu COM a pravidlům standardní modelu COM pro vytváření objektů rozhraní zjišťování a odstraňování objektů.  Použití objektů COM v aplikacích klasické pracovní plochy jazyka C++ je poměrně přímočarý, ale zápis objektu modelu COM je složitější. [Aktivní šablony knihovny (ATL)](../atl/atl-com-desktop-components.md) poskytuje makra a pomocných funkcí, které zjednodušují vývoj COM. Další informace najdete v tématu [desktopové komponenty ATL COM](../atl/atl-com-desktop-components.md).
...
Původní C++ podpora UWP se skládal z (1) C++/CX dialekt C++ rozšíření syntaxe, nebo (2) knihovny Runtime Windows (WRL), která je založena na standardu C++ a modelu COM. Obě C++/CX a WRL jsou stále podporovány. Pro nové projekty, doporučujeme [ C++/WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), která je zcela založena na standardu C++ a poskytuje rychlejší výkon.

## <a name="desktop-bridge"></a>Přemostění na Desktop

Ve Windows 10 můžete balíček vaší existující aplikace klasické pracovní plochy nebo objekt modelu COM jako aplikace pro UPW a přidat funkce UPW, jako je touch nebo volání rozhraní API ze sady moderní rozhraní Windows API. Můžete také přidat aplikace pro UPW pro desktop řešení v sadě Visual Studio a balíček je společně v jednom balíčku a používat rozhraní Windows API pro komunikaci mezi nimi.

Visual Studio 2017 verze 15.4 a novější vám umožňuje vytvoření balíčku projektu aplikace Windows výrazně zjednodušit práci při balení aplikace klasické pracovní plochy. Několik omezení se použijí pro volání registru nebo rozhraní API můžete použít vaši desktopovou aplikaci. V mnoha případech však můžete vytvořit kód alternativní cesty dosáhnout podobné funkce jako při spuštění v balíčku aplikace. Další informace najdete v tématu [přemostění na Desktop](/windows/uwp/porting/desktop-to-uwp-root).
...
Windows *služby* je program, který může běžet na pozadí bez zásahu uživatele nebo vůbec žádné. Tyto programy se nazývají *procesy démon* v systémech UNIX. Další informace najdete v tématu [služby](/windows/desktop/services/services).

## <a name="sdks-libraries-and-header-files"></a>Sady SDK, knihovny a soubory hlaviček

Visual Studio obsahuje knihovny Runtime jazyka C (CRT), standardní knihovny C++ a další knihovny specifické pro společnost Microsoft. Většina zahrnutí složky obsahující soubory hlaviček těchto knihoven jsou umístěny v instalačním adresáři nástroje Visual Studio ve složce \VC\. Soubory hlaviček Windows a CRT se nacházejí ve složce instalace sady Windows SDK.

[Správce balíčků Vcpkg](../build/vcpkg.md) vám umožní snadno nainstalovat stovky open source knihoven třetích stran pro Windows.
...
**Program soubory (x86) \Windows Kits** je výchozím umístěním pro všechny verze sady Windows SDK, které jste nainstalovali.
