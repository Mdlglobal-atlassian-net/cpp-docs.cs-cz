---
title: Použití rozhraní ODBC pro práci s jinými uživateli | Dokumentace Microsoftu
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-data
ms.topic: conceptual
dev_langs:
- C++
helpviewer_keywords:
- ODBC, working with other users
ms.assetid: 8ff18684-f62b-4027-b9b2-7c3b5d52bd89
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
- data-storage
ms.openlocfilehash: 1d2456717083fc70091d5774ff79bf574a1266af
ms.sourcegitcommit: a9dcbcc85b4c28eed280d8e451c494a00d8c4c25
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 10/25/2018
ms.locfileid: "50059503"
---
# <a name="use-odbc-to-work-with-other-users"></a>Použití rozhraní ODBC pro práci s jinými uživateli

Použití rozhraní ODBC v prostředích, kde více uživatelé pracují s databází rozhraní ODBC.

## <a name="in-this-section"></a>V tomto oddílu

## <a name="see-also"></a>Viz také

[Open Database Connectivity (ODBC)](../../data/odbc/open-database-connectivity-odbc.md)