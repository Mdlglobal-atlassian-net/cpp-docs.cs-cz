---
title: DD | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-masm
ms.topic: reference
f1_keywords:
- dd
dev_langs:
- C++
helpviewer_keywords:
- DD directive
ms.assetid: 0c238628-2fe2-437e-979d-a90bdae7b478
author: corob-msft
ms.author: corob
ms.workload:
- cplusplus
ms.openlocfilehash: ff556b6ba25743df05073b56b57611346d316cb4
ms.sourcegitcommit: dbca5fdd47249727df7dca77de5b20da57d0f544
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 04/28/2018
ms.locfileid: "32050672"
---
# <a name="dd"></a>DD
Přiděluje a volitelně inicializuje dvojitou slova (4 bajtů) úložiště pro každou `initializer`. `DD` je synonymum z [DWORD](../../assembler/masm/dword.md).  
  
## <a name="syntax"></a>Syntaxe  
  
```  
[[name]] DD initializer [[, initializer]]...  
```  
  
## <a name="remarks"></a>Poznámky  
 Můžete také použít jako specifikátor typu kdekoli je právní typu.  
  
## <a name="see-also"></a>Viz také  
 [Referenční dokumentace k direktivám](../../assembler/masm/directives-reference.md)