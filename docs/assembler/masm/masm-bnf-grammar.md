---
title: Gramatika BNF shromažďování maker maker maker ů společnosti Microsoft
...
ms.locfileid: "81317964"
---
# <a name="microsoft-macro-assembler-bnf-grammar"></a>Gramatika BNF shromažďování maker maker maker ů společnosti Microsoft

Tato stránka obsahuje popis BNF gramatiky MASM. Je k dispozici jako doplněk k odkazu a není zaručeno, že bude kompletní. Úplné informace o klíčových slovech, parametrech, operacích a tak dále naleznete v odkazu.

Pro ilustraci použití BNF, následující diagram ukazuje definici typedef směrnice, počínaje nonterminal *`typedefDir`*.

![Příklad MASM BNF](media/bnf.png)

Položky pod každou vodorovnou závorku **`FAR16`** jsou **`FAR32`** svorky, například **`NEAR16`**, **`NEAR32`**, a . Nebo se nepoužívají terminály, *`qualifier`* *`qualifiedType`* například , , *`distance`* a *`protoSpec`* které lze dále definovat. Každý kurzíva nonterminal *`typedefDir`* v definici je také položka v BNF. Tři svislé tečky označují definici větvení pro neterminál, která z důvodu jednoduchosti toto číslo neilustruje.

Gramatika BNF umožňuje rekurzivní definice. Například gramatika používá qualifiedType jako možnou definici pro qualifiedType, což je také součástí definice pro kvalifikátor. Symbol |určuje volbu mezi alternativními výrazy, *`endOfLine`*  |  *`comment`* například . Dvojité závorky určují volitelný *`macroParmList`* parametr, například ... Závorky se ve skutečnosti nezobrazují ve zdrojovém kódu.
...
*`character`*\
&nbsp;&nbsp;&nbsp;&nbsp;Libovolný znak s řadovým v rozsahu 0–255 kromě posuvu řádků (10).
...
*`delimiter`*\
&nbsp;&nbsp;&nbsp;&nbsp;Libovolný znak kromě*`whiteSpaceCharacter`*
...
*`id`*\
&nbsp;&nbsp;&nbsp;&nbsp;První znak identifikátoru může být velký nebo malý abecední znak (`[A–Za-z]` `@ _ $ ?` ) nebo některý z těchto čtyř znaků: Zbývající znaky`[0–9]`mohou být libovolný z těchto znaků nebo desetinné číslice ( ). Maximální délka je 247 znaků.
...
*`stringChar`*\
&nbsp;&nbsp;&nbsp;&nbsp;*`quote`**`quote`* | Libovolný znak kromě citace.
...
*`text`*\
&nbsp;&nbsp;&nbsp;&nbsp;*`textLiteral`* | *`text`* charakter | **`!`** *`character`* *`text`* | *`character`* | **`!`***`character`*
