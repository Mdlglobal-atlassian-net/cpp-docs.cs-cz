---
title: Obecná stránka vlastností (soubor)
ms.date: 11/04/2016
f1_keywords:
- VC.Project.VCFileConfiguration.ExcludedFromBuild
- VC.Project.VCFileConfiguration.Tool
ms.assetid: 26e3711e-9e7d-4e8d-bc4c-2474538efdad
ms.openlocfilehash: 66e26cabf5af85091000e6cda144898789c581df
ms.sourcegitcommit: 0ab61bc3d2b6cfbd52a16c6ab2b97a8ea1864f12
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 04/23/2019
ms.locfileid: "62292222"
---
# <a name="general-property-page-file"></a>Obecná stránka vlastností (soubor)

Když je vybrán soubor v **Průzkumníka řešení**, **Obecné** stránka vlastností **vlastnosti konfigurace** uzel obsahuje následující vlastnosti:

- **Vyloučit ze sestavení**

   Určuje, zda soubor by měl být v sestavení pro aktuální konfiguraci.

   Programový přístup k této vlastnosti, najdete v článku <xref:Microsoft.VisualStudio.VCProjectEngine.VCFileConfiguration.ExcludedFromBuild%2A>.

- **Nástroj**

   Nástroj, který se použije k vytvoření tohoto souboru. Zobrazit [zadání Custom Build Tools](../specifying-custom-build-tools.md) Další informace.

   Programový přístup k této vlastnosti, najdete v článku <xref:Microsoft.VisualStudio.VCProjectEngine.VCFileConfiguration.Tool%2A>.

Informace o tom, jak získat přístup k **Obecné** stránka vlastností **vlastnosti konfigurace** uzlu, naleznete v tématu [vlastnosti kompilátoru a sestavení nastavte C++ v sadě Visual Studio](../working-with-project-properties.md).

Projekty bez Windows, naleznete v tématu [odkaz na stránku vlastností Linux C++](../../linux/prop-pages-linux.md).

## <a name="see-also"></a>Viz také:

[Odkaz na stránku vlastností projektu jazyka C++](property-pages-visual-cpp.md)