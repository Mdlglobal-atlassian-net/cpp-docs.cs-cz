...
- První část řetězec ID identifikuje typ členu, identifikují se jeden znak následovaný dvojtečkou. Se používají následující typy členů:
  ...
  | E | event |
  | ! | Text chyby<br /><br /> Zbývající řetězec poskytuje informace o této chybě. Kompilátor MSVC generuje informace o chybě pro odkazy, které nelze rozpoznat. |
...
- Vlastnosti a metody Pokud jsou argumenty metody, uzavřený do závorek seznamu argumentů se řídí. Pokud neexistují žádné argumenty, jsou k dispozici žádné závorky. Argumenty jsou odděleny čárkami. Kódování každý argument následuje přímo jak je zakódovaný v rozhraní .NET Framework podpisu:
  ...
  - ELEMENT_TYPE_PINNED je vyjádřena jako ' ^' následující typ změny. Kompilátor MSVC generuje nikdy to.

  - ELEMENT_TYPE_CMOD_REQ je vyjádřena jako "&#124;" a plně kvalifikovaný název třídy modifikátor následující typ změny. Kompilátor MSVC generuje nikdy to.
  ...
  - "[?]" Představuje ELEMENT_TYPE_GENERICARRAY následující typ prvku pole. Kompilátor MSVC generuje nikdy to.
  ...
  - Typ ELEMENT_TYPE_FNPTR je reprezentován jako "= FUNC:`type`(*podpis*)", kde `type` je návratový typ a *podpis* je argumenty metody. Pokud neexistují žádné argumenty, jsou vynechány závorky. Kompilátor MSVC generuje nikdy to.
