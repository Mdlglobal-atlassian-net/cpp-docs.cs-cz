...
@No__t-0, který obsahuje uzly `ClInclude`, se vytvoří při prvním spuštění projektu. Pokud generujete vlastní soubory vcxproj, ujistěte se, že všechny položky projektu mají také záznam v souboru filters. Hodnoty v uzlu `ClInclude` přepíšou výchozí filtrování na základě přípon souborů. Při použití sady Visual Studio k přidání nové položky do projektu přidá rozhraní IDE jednotlivé položky souboru do souboru filtrů. Když změníte příponu souboru, filtr se automaticky znovu nepřiřazuje. 
