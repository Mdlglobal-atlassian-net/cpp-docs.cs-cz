...
Možnosti kompilátoru pro předkompilované hlavičky jsou [/y](reference/y-precompiled-headers.md). Na stránkách vlastností projektu jsou možnosti umístěny v části **Vlastnosti konfigurace > C/C++ > předkompilovaných hlaviček**. Můžete zvolit, že nebudete používat předkompilované hlavičky, a můžete zadat název souboru hlaviček a cestu k výstupnímu souboru.

## <a name="custom-precompiled-code"></a>Vlastní předkompilovaný kód

U rozsáhlých projektů, které se při sestavování významně dobírají, je vhodné zvážit vytváření vlastních předkompilovaných souborů. Kompilátory Microsoft C a C++ poskytují možnosti pro předkompilování kódu C nebo C++, včetně vloženého kódu. Pomocí této funkce výkonu můžete kompilovat stabilní tělo kódu, ukládat zkompilovaný stav kódu v souboru a během následných kompilací kombinovat předkompilovaný kód s kódem, který je stále ve vývoji. Každá následná kompilace je rychlejší, protože stabilní kód není nutné znovu kompilovat.
...
Můžete předem zkompilovat kód jazyka C nebo C++; Nejste omezeni na předkompilování pouze hlavičkových souborů.
...
Direktivy pragma zpracované během vytváření souboru PCH obvykle ovlivňují soubor, se kterým se následně používá soubor PCH. `comment` Direktivy `message` pragma a neovlivňují zbývající část kompilace.
...
> [!NOTE]
> Zařízení předkompilovaných hlaviček je určeno pro použití pouze ve zdrojových souborech jazyka C a C++.
...
Soubory, které jsou uvedeny v těchto makrech, mohou být hlavičkové soubory nebo zdrojové soubory jazyka C nebo C++. (Jeden soubor PCH nelze použít s moduly C a C++.) Všimněte si, že můžete použít makro **hdrstop** k zastavení předkompilace v určitém bodě v souboru BOUNDRY. Další informace najdete v tématu [hdrstop](../preprocessor/hdrstop.md) .
