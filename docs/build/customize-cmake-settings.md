...
Odpovídá nastavení **názvu** . Tento název se zobrazí v rozevíracím seznamu konfigurace C++. `${name}` Makro lze použít k vytvoření dalších hodnot vlastností, jako jsou například cesty.
...
Odpovídá **proměnným**. Obsahuje dvojici název-hodnota proměnných cmake předaných **jako** *hodnota _názvu_=* pro cmake. Pokud vaše pokyny pro sestavení vašeho projektu CMake určují přidání jakýchkoli proměnných přímo do souboru mezipaměti CMake, doporučujeme je místo toho přidat.
